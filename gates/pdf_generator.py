...
class CodeGatesPDFGenerator:
    ...
    def _build_recommendations_section(self, gate: Dict[str, Any]) -> List:
        """Build AI recommendations section like HTML report"""
        elements = []
        
        # Only use LLM recommendations now, not the old non-LLM ones
        llm_recommendation = gate.get("llm_recommendation")
        if not llm_recommendation:
            return elements
        
        # Only show LLM recommendations for FAIL/WARNING gates
        status = gate.get("status", "")
        if status not in ["FAIL", "WARNING"]:
            return elements
        
        elements.append(Paragraph("AI Recommendations", self.custom_styles['Heading1']))
        
        # Add the LLM recommendation content
        elements.append(Paragraph(llm_recommendation, self.custom_styles['Normal']))

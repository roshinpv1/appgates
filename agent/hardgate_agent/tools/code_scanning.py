...
def scan_code(repository_path: str, scan_types: Optional[List[str]] = None, scan_depth: str = "comprehensive", tool_context = None) -> Dict[str, Any]:
    """
    Scan code for vulnerabilities, secrets, and security issues.
    
    Args:
        repository_path: Path to the repository to scan
        scan_types: Types of scans to perform (vulnerabilities, secrets, dependencies, configuration, authentication)
        scan_depth: Depth of scanning (minimal, standard, comprehensive)
        tool_context: The ADK tool context
    
    Returns:
        Dictionary containing code scanning results
    """
    try:
        if not repository_path:
            return {
        ...
        scan_types_to_use = scan_types or ["vulnerabilities", "secrets", "dependencies", "configuration", "authentication"]
        
        # Perform different types of scans
        scan_results = {}
        
        if "vulnerabilities" in scan_types_to_use:
            scan_results["vulnerabilities"] = _scan_vulnerabilities(repository_path, scan_depth)
        
        if "dependencies" in scan_types_to_use:
            scan_results["dependencies"] = _scan_dependencies(repository_path)
        
        if "configuration" in scan_types_to_use:
            scan_results["configuration"] = _scan_configuration(repository_path)
        
        if "secrets" in scan_types_to_use:
            scan_results["secrets"] = _scan_secrets(repository_path, scan_depth)
        
        if "authentication" in scan_types_to_use:
            scan_results["authentication"] = _scan_authentication(repository_path, scan_depth)
        
        # Generate summary
        summary = _generate_scan_summary(scan_results)
        
        return {
            "success": True,
            "repository_path": repository_path,
            "scan_types": scan_types_to_use,
            "scan_results": scan_results,
            "summary": summary
        }
...
def _scan_vulnerabilities(repo_path: str, scan_depth: str) -> dict:
    """Scan for code vulnerabilities using real patterns"""
    vulnerabilities = []
    
    # Comprehensive vulnerability patterns
    vulnerability_patterns = [
        {"pattern": r"eval\s*\(", "type": "Code Injection", "severity": "High", "description": "Dangerous eval() function usage"},
        {"pattern": r"exec\s*\(", "type": "Code Injection", "severity": "High", "description": "Dangerous exec() function usage"},
        {"pattern": r"sql.*injection|sql.*query.*\$\{|sql.*query.*%s", "type": "SQL Injection", "severity": "High", "description": "Potential SQL injection vulnerability"},
        {"pattern": r"xss|cross.*site.*scripting|innerHTML.*=.*\$\{|document\.write.*\$\{", "type": "XSS", "severity": "Medium", "description": "Potential Cross-Site Scripting vulnerability"},
        {"pattern": r"password.*=.*['\"][^'\"]*['\"]", "type": "Hardcoded Password", "severity": "High", "description": "Hardcoded password found"},
        {"pattern": r"api.*key.*=.*['\"][^'\"]*['\"]", "type": "Hardcoded API Key", "severity": "High", "description": "Hardcoded API key found"},
        {"pattern": r"http://(?!localhost|127\.0\.0\.1)", "type": "Insecure Protocol", "severity": "Medium", "description": "Insecure HTTP protocol usage"},
        {"pattern": r"console\.log.*password|logger.*password", "type": "Password Logging", "severity": "Medium", "description": "Password being logged"},
        {"pattern": r"innerHTML.*=.*\$\{|document\.write.*\$\{", "type": "Template Injection", "severity": "Medium", "description": "Potential template injection"},
        {"pattern": r"subprocess\.call|os\.system|subprocess\.Popen", "type": "Command Injection", "severity": "High", "description": "Potential command injection"},
        {"pattern": r"pickle\.loads|pickle\.load", "type": "Insecure Deserialization", "severity": "High", "description": "Insecure deserialization using pickle"},
        {"pattern": r"random\.randint|Math\.random", "type": "Weak Random", "severity": "Low", "description": "Weak random number generation"},
        {"pattern": r"md5|sha1", "type": "Weak Hashing", "severity": "Medium", "description": "Weak cryptographic hash function"},
        {"pattern": r"base64\.decode|atob", "type": "Weak Encoding", "severity": "Low", "description": "Weak encoding used for sensitive data"},
        {"pattern": r"localStorage|sessionStorage.*password", "type": "Insecure Storage", "severity": "Medium", "description": "Sensitive data stored in browser storage"}
    ]
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for file in files:
            if _should_scan_file(file, scan_depth):
                file_path = os.path.join(root, file)
                file_vulns = _scan_file_for_vulnerabilities(file_path, vulnerability_patterns)
                vulnerabilities.extend(file_vulns)
...
def _scan_dependencies(repo_path: str) -> dict:
    """Scan for dependency security issues"""
    dependency_issues = []
    ...
    dependency_files = [
        "package.json", "requirements.txt", "pom.xml", "build.gradle", 
        "go.mod", "Cargo.toml", "Gemfile", "composer.json", "yarn.lock", "package-lock.json"
    ]
    
    for dep_file in dependency_files:
        file_path = os.path.join(repo_path, dep_file)
        if os.path.exists(file_path):
            issues = _analyze_dependency_file(file_path)
            dependency_issues.extend(issues)
...
def _scan_configuration(repo_path: str) -> dict:
    """Scan for configuration security issues"""
    config_issues = []
    
    # Check for configuration files
    config_files = [
        ".env", "config.json", "application.properties", "application.yml", "application.yaml",
        "docker-compose.yml", "Dockerfile", "kubernetes.yaml", "k8s.yaml", "helm", "terraform",
    ...
    for config_file in config_files:
        file_path = os.path.join(repo_path, config_file)
        if os.path.exists(file_path):
            issues = _analyze_config_file(file_path)
            config_issues.extend(issues)
...
def _scan_secrets(repo_path: str, scan_depth: str) -> dict:
    """Scan for secrets and sensitive data"""
    secrets_found = []
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for file in files:
            if _should_scan_file(file, scan_depth):
                file_path = os.path.join(root, file)
                file_secrets = _scan_file_for_secrets(file_path, secret_patterns)
                secrets_found.extend(file_secrets)
...
def _scan_authentication(repo_path: str, scan_depth: str) -> dict:
    """Scan for authentication and authorization issues"""
    auth_issues = []
    
    # Authentication patterns
    auth_patterns = [
        {"pattern": r"authentication.*disabled|auth.*disabled", "type": "Auth Disabled", "severity": "High", "description": "Authentication disabled"},
        {"pattern": r"authorization.*disabled|authz.*disabled", "type": "AuthZ Disabled", "severity": "High", "description": "Authorization disabled"},
        {"pattern": r"skip.*auth|bypass.*auth|skip.*authentication", "type": "Auth Bypass", "severity": "High", "description": "Authentication bypass found"},
        {"pattern": r"admin.*=.*true|isAdmin.*=.*true", "type": "Hardcoded Admin", "severity": "High", "description": "Hardcoded admin privileges"},
        {"pattern": r"role.*=.*admin|isAdmin.*=.*true", "type": "Hardcoded Admin Role", "severity": "High", "description": "Hardcoded admin role"},
        {"pattern": r"jwt.*secret.*=.*['\"][^'\"]*['\"]", "type": "Weak JWT Secret", "severity": "Medium", "description": "Weak JWT secret"},
        {"pattern": r"password.*=.*['\"][^'\"]{1,7}['\"]", "type": "Weak Password", "severity": "Medium", "description": "Weak password found"},
        {"pattern": r"no.*password|empty.*password", "type": "No Password", "severity": "High", "description": "No password protection"},
        {"pattern": r"guest.*=.*true|anonymous.*=.*true", "type": "Guest Access", "severity": "Medium", "description": "Guest/anonymous access enabled"}
    ]
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for file in files:
            if _should_scan_file(file, scan_depth):
                file_path = os.path.join(root, file)
                file_auth_issues = _scan_file_for_auth_issues(file_path, auth_patterns)
                auth_issues.extend(file_auth_issues)
    
    return {
        "total_issues": len(auth_issues),
        "authentication_issues": auth_issues,
        "severity_breakdown": _get_severity_breakdown(auth_issues)
    }


def _should_scan_file(filename: str, scan_depth: str) -> bool:
    """Determine if file should be scanned based on scan depth"""
...
def _scan_file_for_vulnerabilities(file_path: str, patterns: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Scan a file for vulnerabilities"""
    vulnerabilities = []
...
def _scan_file_for_secrets(file_path: str, patterns: List[str]) -> List[Dict[str, Any]]:
    """Scan a file for secrets"""
    secrets = []
...
def _scan_file_for_auth_issues(file_path: str, patterns: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Scan a file for authentication issues"""
    issues = []
...
def _analyze_dependency_file(file_path: str) -> List[Dict[str, Any]]:
    """Analyze dependency file for security issues"""
    issues = []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            vulnerable_packages = [
                "lodash", "jquery", "moment", "bootstrap", "angular", "react", "vue"
            ]
...
def _analyze_config_file(file_path: str) -> List[Dict[str, Any]]:
    """Analyze configuration file for security issues"""
    issues = []
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if "ssl.*=.*false" in content.lower() or "https.*=.*false" in content.lower():
                issues.append({
                    "file": file_path,
                    "type": "SSL Disabled",
                    "severity": "High",
                    "description": "SSL/HTTPS disabled in configuration"
                })
            ...
            if "cors.*=.*\*" in content.lower():
                issues.append({
                    ...
                    "type": "Open CORS",
                    "severity": "Medium",
                    "description": "Open CORS policy configured"
                })
...
def _generate_scan_summary(scan_results: Dict[str, Any]) -> dict:
    """Generate summary of all scan results"""
    total_issues = 0
    high_severity = 0
    medium_severity = 0
    ...
    return {
        ...
        "medium_severity": medium_severity,
        "low_severity": low_severity,
        "risk_level": _calculate_risk_level(high_severity, medium_severity),
        "recommendations": _generate_scan_recommendations(scan_results)
    }


def _calculate_risk_level(high_count: int, medium_count: int) -> str:
    """Calculate overall risk level"""
    if high_count > 5:
        return "Critical"
    elif high_count > 0 or medium_count > 10:
...
def _generate_scan_recommendations(scan_results: Dict[str, Any]) -> List[str]:
    """Generate recommendations based on scan results"""
    recommendations = []
